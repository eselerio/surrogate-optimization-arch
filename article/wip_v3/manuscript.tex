...
$f_{\rm ns}$ & Non-settleable fraction in the Clarifier settling law\\
...
$X_{\rm feed}=X_N$ & TSS concentration entering the Clarifier\\
...
Activated Sludge Models describe this coupling mechanistically, while multilayer settling models represent clarification and thickening \citep{Henze2006,Takacs1991}. In the Tak\'acs settling law, the parameter $f_{\rm ns}$ identifies a feed-dependent concentration scale below which solids are treated as non-settleable. This low-concentration behavior matters at the treated-water outlet: non-negativity alone permits an arbitrarily small positive overflow TSS even when the feed carries an irreducible non-settleable fraction. Repeatedly integrating the mechanistic equations to a stable steady state is expensive inside operational optimization. A statistical surrogate can be evaluated much faster, but an unconstrained regression may predict a negative concentration, lose a conserved inventory between reactors, create component mass in the Clarifier, or overstate clarification by driving overflow TSS below that scale. Optimization amplifies this risk because its purpose is to search for unusually favorable predictions \citep{Bhosekar2018}.

The present formulation gives regression and physics different, auditable roles. It extends the invariant-aware ICSOR surrogate of \citet{Selerio2026}, which uses a second-order regression and invariant-constraint projection, from its original response setting to the complete recycling plant. The extended ICSOR predicts the complete decision-relevant closed-loop response in one quadratic map. Internal Clarifier layers are omitted from the statistical response because they are not needed to close the recycle, calculate outlet quality, or evaluate the operating objective; their volume-weighted total is retained because whole-plant solids retention time depends on Clarifier inventory. A strictly convex quadratic program (QP) finds the nearest response satisfying the stated linear physical relations, including a scalar lower bound that prevents predicted overflow TSS from falling below the declared non-settleable fraction of feed TSS. The extended ICSOR and the smooth mechanistic NLP use identical controls, objective, and engineering limits. Apart from this reduced endpoint closure, nonlinear biological rates and the nonlinear settling law are not attributed to the projection; they are checked independently with the full layer-resolved state of the declared aggregate-solids mechanistic model. This distinction is important because satisfying conservation, the non-settleable floor, and aggregate inventory bounds is necessary for a credible plant response, but it is not sufficient to establish kinetic or layer-profile accuracy.
...
Define the Clarifier-feed TSS as $X_{\rm feed}:=X_N=t_X^\top c_N$. The Tak\'acs double-exponential settling relation is evaluated using an effective solids concentration displaced by $f_{\rm ns}X_{\rm feed}$ and clipped to non-negative settling velocity \citep{Takacs1991}. This construction represents a fraction of the feed solids that is not removed by gravity settling. In a reduced endpoint model that does not carry the layer fluxes, its relevant consequence is declared explicitly as
\begin{equation}
X_E\ge f_{\rm ns}X_{\rm feed}=f_{\rm ns}X_N,
\qquad 0\le f_{\rm ns}<1.
\label{eq:nonsettleablefloor}
\end{equation}
Because $q_E>0$, the equivalent division-free form is
\begin{equation}
q_Ef_{\rm ns}t_X^\top c_N-t_X^\top g_E\le0.
\label{eq:nonsettleableflow}
\end{equation}
Equation~\eqref{eq:nonsettleablefloor} prevents the regression and optimizer from representing complete or near-complete solids removal when the Clarifier feed contains a declared non-settleable fraction. It is a conservative scalar endpoint closure motivated by the settling law, not a replacement for its nonlinear flux function or layer balances.
...
\begin{array}{c}
q_UE_Xc_N-E_Xg_U\\
q_Ef_{\rm ns}t_X^\top c_N-t_X^\top g_E\\
...
\label{eq:G}
\end{equation}
Thus $n_g=F_X+3$ inequality rows describe particulate densification, the non-settleable overflow floor, and aggregate-inventory feasibility. The scalar row after particulate densification is Equation~\eqref{eq:nonsettleableflow}; the last two rows are the division-free form of Equation~\eqref{eq:inventoryenvelope}. All remain linear in $\chi$ at fixed $\vartheta$. Non-negativity contributes another $n_\chi$ inequalities.
...
\label{eq:projection}\\
\text{subject to}\quad&
D_b^{-1}\{\mathcal H(\vartheta)[\chi_{{\rm raw},S}+D_\chi u]-\mathfrak b(\vartheta,x)\}=0,\notag\\
...
$f_{\rm ns}$ & 0.00228 & Non-settleable TSS fraction\\
...
On 26 August 2026, after descriptive low-TSS prediction diagnostics had been inspected, the invariant-constraint projection was revised to include Equation~\eqref{eq:nonsettleablefloor}. The diagnostic showed that conservation, densification, inventory bounds, and non-negativity alone still permit a reduced response with overflow TSS below the non-settleable scale represented in the mechanistic settling law. This outcome-aware methodological revision leaves the second-order response definition, statistical inputs, and method names unchanged, but adds one inequality row to the projection polyhedron. It therefore changes projected predictions, active sets and sensitivities, projection-correction and trust diagnostics, and any optimization or certification that depends on the projected response. Earlier projected assessments and route-$S$ optimization results are consequently superseded and must be recomputed under the revised projection. The frozen post-selection holdout and ten scenario inputs remain descriptive rather than confirmatory evidence.
...
The frozen post-selection holdout reports descriptive RMSE, MAE, bias, normalized RMSE, and $R^2$ for the extended-ICSOR mixer, each reactor, both outlet-flow blocks, Clarifier solids inventory, and full reduced response. Raw and projected assembled predictions are reported side by side. The report additionally gives the raw and projected counts and maximum magnitudes of violations of Equation~\eqref{eq:nonsettleablefloor}, together with the fraction of projected rows for which the floor is active within the declared numerical tolerance. For holdout row $i$, let $\mathcal C_i$ be its projection polyhedron and let $P_{\mathcal C_i}$ be metric projection in the norm $\|v\|_{D_\chi^{-1}}:=\|D_\chi^{-1}v\|_2$. Because a numerical mechanistic target $\chi_i$ satisfies equations only to finite tolerance, its feasibility distance $\delta_{{\rm feas},i}=\|P_{\mathcal C_i}(\chi_i)-\chi_i\|_{D_\chi^{-1}}$ is reported. The following general bound is checked for every target:
\begin{equation}
\|P_{\mathcal C_i}(\chi_{{\rm raw},S,i})-\chi_i\|_{D_\chi^{-1}}
\le \|\chi_{{\rm raw},S,i}-\chi_i\|_{D_\chi^{-1}}+\delta_{{\rm feas},i}.
\label{eq:finitefeasibility}
\end{equation}
Equation~\eqref{eq:nonexpansive} is invoked only for a target known to be exactly feasible; a merely small positive $\delta_{{\rm feas},i}$ does not justify the stronger squared inequality. Extended ICSOR retains its existing admission gates: the development out-of-fold QPs must pass primal--dual audits, full assembled-response and inventory-coordinate raw nRMSE values must remain below one, and every trust limit must be frozen before optimization. The separate inventory criterion prevents its single coordinate from being hidden in the 161-coordinate average even though it enters SRT directly. Holdout metrics are descriptive and do not alter these gates. A failed scientific gate does not trigger refitting. A non-finite or incomplete numerical object from which a downstream problem cannot be defined remains a hard computational failure.
...
\end{align}
Each valid, physically audited exact replay retains its exact objective and objective components even when the exact response is engineering-infeasible. Such a candidate is reported but cannot enter the paired comparison. The difference in Equation~\eqref{eq:comparisons} is eligible only when both candidates have valid, engineering-feasible exact replays. These quantities compare selected feasible incumbents and are not called regret because neither route supplies a global certificate. A symmetric relative objective difference and normalized control RMS and maximum differences accompany every eligible pair.
...
\begin{table}[htbp]
\centering
\caption{Plant-grouped five-fold selection and descriptive post-selection holdout performance for extended ICSOR. Raw and projected errors are required for every assembled reduced-response block.}
...
\begin{table}[htbp]
\centering
\caption{Required scope-specific feasibility and nonlinear audit. Each entry reports the applicable maximum residual and exceedance count; the non-settleable-floor entry also reports the active-row fraction. n.a. means that the source has no layer state on which to evaluate that quantity.}
\label{tab:scope_audit_results}
\scriptsize
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcccccc}
\toprule
Source & Non-settleable floor & Inventory bounds & Reactor residual & Layer envelope & Layer residual & Stability\\
\midrule
Extended ICSOR raw reduced & -- & -- & -- & n.a. & n.a. & n.a.\\
...
Clarifier floor ratio $X_E/(f_{\rm ns}X_N)$ & -- & -- & -- & --\\
...
The nominal discussion will trace raw, projected, optimizer-native, and exact responses from the mixer through the reactors, outlet streams, and $M_{\rm cl}$. Detailed Clarifier layers will be discussed only for smooth-direct and exact mechanistic states. The discussion will identify which inventories required correction, which controls or engineering constraints were active, whether any replay carries a branch-boundary qualifier, and whether the two exact-reference objectives differ materially.
