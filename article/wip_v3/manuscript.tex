...
\item A primal--dual gap yields a smaller single-level optimization model.
...
\subsection{An exact lower-level certificate without complementarity slacks}
...
The single-level calculation replaces the last line of Equation~\eqref{eq:bilevel} by primal feasibility, $\lambda^{\le}\ge0$, and the dimension-normalized condition $\overline\Gamma=\Gamma/(n_\chi+n_q)\le\tau$. A decreasing positive sequence of $\tau$ values is used for numerical continuation. The final controls are accepted only after the projection QP is solved independently with a convex-QP method \citep{Stellato2020} and its primal feasibility, dual feasibility, stationarity, complementarity, and upper-level constraints all pass their stated tolerances. Multiple deterministic starts seek distinct local basins; they are not a global-optimality certificate.
...
\begin{enumerate}
\item generate 1,000 accepted mechanistic steady states over the joint seven-control and twenty-influent domain;
\item place 800 states in the development block and 200 independently randomized states in an untouched test block;
\item select the ridge parameter by five-fold cross-validation entirely within the 800 development states and fit one final development surrogate;
\item evaluate raw and projected responses once on the untouched 200 states;
...
\end{split}
\end{equation}
Independent randomized Latin-hypercube blocks supply the 800 development and 200 test inputs in 27 dimensions \citep{McKay1979}. Rows within a block are stratified and are not described as independent observations. A failed or unstable mechanistic point is not silently replaced; an unresolved design point stops the study so solver success cannot condition the intended design.

\subsection{Five-fold cross-validation and final ridge fit}

The 800 development rows are partitioned reproducibly into five folds of 160. For a fitting index set $\mathcal I$, let $\Phi_{\mathcal I}$ contain feature rows, $\widetilde Y_{\mathcal I}$ contain standardized responses, and $R_\phi=\diag(0,1,\ldots,1)$ leave the intercept unpenalized. Training solves
\begin{equation}
...
No numerical result is inserted before the complete 1,000-state design and all eleven paired optimization cases have been attempted and adjudicated under the frozen checks. Failed or unresolved cases remain visible in the denominators and result tables rather than being replaced or omitted. Tables~\ref{tab:prediction_results}--\ref{tab:timing_results} define the minimum result set and prevent selective reporting after outcomes are known.
...
\begin{table}[htbp]
\centering
\caption{Smooth-NLP versus reference nonsmooth comparison over the 200 test states and every available selected decision (at most 22); the actual denominator is reported.}
...
Total evaluated fixed-input points, $200+n_{\rm sel}$ & \multicolumn{3}{c}{--}\\
