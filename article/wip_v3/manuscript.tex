...
\begin{abstract}
Activated-sludge optimization is difficult because biological conversion, internal mixed-liquor recycle, return activated sludge, wasting, and secondary clarification form one nonlinear closed loop. This work formulates the system for an arbitrary number $N$ of completely mixed reactors and an arbitrary number $L$ of mechanistic Clarifier layers. The freely varied coordinates may include stage residence times, independent stage aeration settings, recycle ratios, wasting, and other explicitly balanced stage inputs. A quadratic multiresponse surrogate predicts the mixer, every reactor outlet, both Clarifier outlet component-flow vectors, and one aggregate Clarifier-solids inventory. Its $n_\chi=(N+3)F+1$ responses are reconciled by a unique scaled-Euclidean projection that imposes recycle mixing, stoichiometric-invariant transport, Clarifier component conservation, soluble pass-through, particulate densification, tight outlet-derived inventory bounds, and non-negativity. The statistical dimension is therefore independent of the number of layers used by the mechanistic settling model. Operational optimization is constrained by this projected response. A seven-variable local optimizer cold-solves the exact projection quadratic program at each distinct trial control and primarily obtains total response derivatives from a verified active-set KKT sensitivity system. When that derivative system is unavailable, deterministic value-only optimization continues over the unchanged QP. Surrogate convergence is qualified either by a strong first-order KKT certificate or by complete two-scale feasible no-descent polls that make no classical-stationarity claim. The case study uses five equal-volume reactors, independently adjustable aeration in reactors 3--5, a ten-layer mechanistic Clarifier, 16,714 accepted mechanistic design states, five-fold cross-validation, and ten post-selection influent scenarios. Every selected surrogate and direct candidate is replayed on the same exact nonsmooth, layer-resolved aggregate-solids model; shared reduced responses and full layer-resolved mechanistic states receive their applicable physical audits. The formulation separates regression error, physical reconciliation, decision quality, smoothing effects, and computational cost without asserting layer-profile prediction or global optimality.
\end{abstract}

\begin{highlights}
\item Surrogate size is independent of the mechanistic Clarifier layer count.
\item Stage residence time and aeration can be manipulated independently.
\item A unique convex projection reconciles the reduced operational response.
\item Surrogate convergence has strong-KKT and finite-resolution poll tiers.
\item Every selected decision is judged on one exact ten-layer reference model.
\end{highlights}
...
$n_\chi,n_y,n_\phi,d_z$ & Surrogate-response, mechanistic-state, feature, and statistical-input dimensions\\
...
$\chi_{\rm raw},\widehat\chi$ & Raw-surrogate and physically projected operational responses\\
...
The present formulation gives regression and physics different, auditable roles, extending invariant-aware surrogate modeling from isolated responses to the recycling plant \citep{Selerio2026}. A quadratic multiresponse model predicts the decision-relevant closed-loop response. Internal Clarifier layers are omitted from that statistical response because they are not needed to close the recycle, calculate outlet quality, or evaluate the operating objective; their volume-weighted total is retained because whole-plant solids retention time depends on Clarifier inventory. A strictly convex quadratic program (QP) then finds the nearest response satisfying a stated set of linear physical relations. The optimizer uses only this projected response. Nonlinear biological rates and the nonlinear settling law are not attributed to the projection; they are checked independently with the full layer-resolved state of the declared aggregate-solids mechanistic model. This distinction is important because satisfying conservation and aggregate inventory bounds is necessary for a credible plant response, but it is not sufficient to establish kinetic or layer-profile accuracy.
...
\subsection{Raw operational-response surrogate}

The regression predicts every quantity needed to close the recycle and evaluate the objective and operating constraints, without reproducing latent spatial detail. Stack the outputs in process order:
\begin{equation}
...
The correction is measured in standardized response coordinates so that the large-magnitude inventory coordinate does not overwhelm a small nutrient coordinate. The projected displacement and response are
\begin{align}
...
Physical feasibility alone does not prevent optimization from asking the projection to make an unusually large repair or from exploiting a state that conserves inventories but is kinetically implausible. Four development-supported diagnostics therefore define $g_{\rm trust}\le0$: standardized correction $\kappa_{\rm corr}=\|\widehat u\|_2/\sqrt{n_\chi}$; regularized feature leverage; deviation from a common particulate recovery; and the scaled smooth reactor-balance residual. Their explicit formulas and thresholds are provided in the Supplementary Material. Each threshold is determined from development data without using the post-selection holdout or optimization responses. No scalar aggregate can reproduce the discarded layer-flux residual, so nonlinear Clarifier plausibility is deliberately reserved for the exact $L$-layer replay rather than represented by a surrogate trust statistic.
...
For this case, $d_z=d_\vartheta+F=27$, $n_\phi=406$, $n_\chi=161$, $n_y=110$, $n_h=75$, $n_g=12$, and $n_q=173$. These are evaluations of Equations~\eqref{eq:state}, \eqref{eq:featurecount}, \eqref{eq:eqcount}, and \eqref{eq:G}; they are not fixed architectural constants. Changing $N$ or the free-control set changes the statistical dimensions and requires a newly fitted surrogate. Changing $L$ changes $n_y$, the mechanistic discretization, and the deterministic construction of $M_{\rm cl}$, but not $n_\chi$; it still requires renewed mechanistic and response validation.
...
\item replace each layer-resolved statistical target by the reduced response in Equation~\eqref{eq:state}, deriving $M_{\rm cl}$ deterministically from the stored mechanistic layers, then select the ridge parameter by five-fold cross-validation entirely within the 13,371 development states and fit one final reduced-response surrogate;
\item evaluate raw and projected reduced responses once on the 3,343 post-selection holdout states for descriptive assessment only;
...
The frozen post-selection holdout reports descriptive RMSE, MAE, bias, normalized RMSE, and $R^2$ for the mixer, each reactor, both outlet-flow blocks, Clarifier solids inventory, and the full reduced response. Raw and projected predictions are reported side by side. For holdout row $i$, let $\mathcal C_i$ be its projection polyhedron and let $P_{\mathcal C_i}$ be metric projection in the norm $\|v\|_{D_\chi^{-1}}:=\|D_\chi^{-1}v\|_2$. Because a numerical mechanistic target $\chi_i$ satisfies equations only to finite tolerance, its feasibility distance $\delta_{{\rm feas},i}=\|P_{\mathcal C_i}(\chi_i)-\chi_i\|_{D_\chi^{-1}}$ is reported. The following general bound is checked for every target:
\begin{equation}
...
\end{equation}
Equation~\eqref{eq:nonexpansive} is invoked only for a target known to be exactly feasible; a merely small positive $\delta_{{\rm feas},i}$ does not justify the stronger squared inequality. Before the post-selection holdout is evaluated, article eligibility requires all development out-of-fold QPs to pass their primal--dual audit, both the full reduced-response and inventory-coordinate raw nRMSE values to be below one, and all four development-supported trust thresholds to be frozen successfully. The separate inventory criterion prevents its single coordinate from being hidden in the 161-coordinate average even though it enters SRT directly. Holdout metrics are descriptive and do not alter these gates. A failed scientific gate is persisted and propagated as a failure status but does not suppress the remaining model-function exercises or trigger refitting. A non-finite or incomplete numerical object from which a downstream problem cannot be defined remains a hard computational failure.
...
\begin{table}[htbp]
\centering
\caption{Five-fold selection and descriptive post-selection holdout performance. Raw and projected errors are required for every reduced-response block.}
\label{tab:prediction_results}
\scriptsize
\begin{tabular}{lrrrrr}
\toprule
Block & CV raw & CV projected & Holdout raw & Holdout projected & Correction RMS\\
\midrule
Mixer & -- & -- & -- & -- & --\\
...
\begin{table}[htbp]
\centering
\caption{Required trust-region audit. Post-selection holdout summaries are descriptive; development-frozen limits constrain surrogate optimization.}
\label{tab:trust_results}
\small
\begin{tabular}{lrrr}
\toprule
Diagnostic & Frozen limit & Holdout 95th percentile & Largest selected value\\
\midrule
...
\midrule
Raw reduced & Post-selection holdout and selected decisions & -- & -- & -- & -- & --\\
Projected reduced & Post-selection holdout and selected decisions & -- & -- & -- & -- & --\\
...
\midrule
Raw reduced & -- & -- & n.a. & n.a. & n.a.\\
Projected reduced & -- & -- & n.a. & n.a. & n.a.\\
...
\begin{table}[htbp]
\centering
\caption{Required nominal response and mechanistic-profile comparison. The final table contains separate panels at $\vartheta_S$ and $\vartheta_M$. Shared response rows are available to all methods; layer rows exist only for smooth-direct and exact mechanistic states.}
...
The nominal discussion will trace raw, projected, optimizer-native, and exact shared responses from the mixer through the reactors, outlet streams, and $M_{\rm cl}$. Detailed Clarifier layers will be discussed only for smooth-direct and exact mechanistic states. The discussion will identify which inventories required correction, which controls or engineering constraints were active, whether either replay carries a branch-boundary qualifier, and whether the two exact-reference objectives differ materially.

\subsection{Casewise exact common-reference validation}

\begin{table}[htbp]
\centering
\caption{Exact nonsmooth common-reference adjudication of every available selected candidate (at most 22). Counts use the actual candidate denominator; the post-selection holdout block is not part of this validation.}
...
\begin{table}[htbp]
\centering
\caption{Required post-selection influent-scenario comparison. The full table contains ten rows. Here $\mathrm{RMS}_{\rm proj,S}=\|e_{\rm proj,S}\|_2/\sqrt{n_\chi}$ and $\mathrm{RMS}_{\rm nat,M}=\|e_{\rm nat,M}\|_2/\sqrt{n_\chi}$ use the reduced $n_\chi=161$ response, and the time ratio is complete mechanistic-route time divided by complete surrogate-route time. Reference objective comparisons are shown only for common-reference-eligible pairs.}
