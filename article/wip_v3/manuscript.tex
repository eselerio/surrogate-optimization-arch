...
$D_\chi,D_R,D_C,D_y,D_b,D_g,D_T$ & Positive plant-response, reactor-response, Clarifier-response, mechanistic-state, physical-row, and quality scales\\
...
$\mathcal R,\mathcal C,\mathcal F_U$ & Shared reactor map, Clarifier map, and shared-unit recycle-closure residual\\
...
\begin{align}
c_{i,U}^{\rm raw}&=\mathcal R_{\gamma_{\rm rxn}}
  (c_{i-1,U}^{\rm raw},d_i,a_i),\qquad i=1,\ldots,N,
\notag\\
\col(g_{E,U}^{\rm raw},g_{U,U}^{\rm raw},M_{{\rm cl},U}^{\rm raw})
&=\mathcal C_{\gamma_{\rm cl}}(c_{N,U}^{\rm raw},r_R,w),
\qquad c_{0,U}^{\rm raw}=m_U^{\rm raw}.
\label{eq:unitmaps}
\end{align}
Here $\mathcal R_{\gamma_{\rm rxn}}:\R^{F+2}\to\R^F$ and $\mathcal C_{\gamma_{\rm cl}}:\R^{F+2}\to\R^{2F+1}$ use separately standardized quadratic feature vectors $\phi_R$ and $\phi_C$, response scales $D_R$ and $D_C$, and independently selected ridge parameters. Sharing $\mathcal R$ assumes identical CSTR kinetics and parameterization; heterogeneous units would require declared unit descriptors or separate fits.
...
\text{subject to}\quad&\vartheta\in\mathcal V,\\
&\mathcal F_U(m_U^{\rm raw};\vartheta,x)=0,\\
...
\midrule
$S,U$ & Mixer & -- & -- & -- & -- & --\\
$S,U$ & Reactors 1--$N$ (each reported) & -- & -- & -- & -- & --\\
$S,U$ & Clarifier overflow & -- & -- & -- & -- & --\\
$S,U$ & Clarifier underflow & -- & -- & -- & -- & --\\
$S,U$ & Clarifier solids inventory $M_{\rm cl}$ & -- & -- & -- & -- & --\\
$S,U$ & Full reduced response & -- & -- & -- & -- & --\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
$S,U$ & Projection correction $\kappa_{\rm corr}$ & -- & -- & --\\
$S$ & Whole-system leverage $\ell_S$ & -- & -- & --\\
$U$ & Reactor leverage $\ell_{U,R,i}$, maximum over stages & -- & -- & --\\
$U$ & Clarifier leverage $\ell_{U,C}$ & -- & -- & --\\
$S,U$ & Particulate split $\kappa_{\rm split}$ & -- & -- & --\\
$S,U$ & Reactor residual $\kappa_{\rm rxn}$ & -- & -- & --\\
...
\begin{tabular}{lrrrr}
\toprule
Location and reported quantity & Raw & Projected & Optimizer-native & Exact\\
\midrule
Mixer: COD, TN, TP, TSS & -- & -- & -- & --\\
...
Mechanistic layer $\ell=1,\ldots,L$: TSS & n.a. & n.a. & direct only & --\\
\bottomrule
\end{tabular}
\end{table}
...
The formulation represents an $N$-stage recycling mixer--reactor--Clarifier plant with a reduced operational response and a unique physical projection. Outlet component flows determine the effluent and underflow TSS endpoints, while the one retained Clarifier scalar, $M_{\rm cl}$, supplies the storage term needed for whole-plant SRT. The statistical dimension therefore follows from $N$, the component partition, and the freely varied controls but not from the number of mechanistic Clarifier layers. Stagewise aeration and residence-time decisions remain supported without treating a five-stage or ten-layer arrangement as universal.

The projection has a deliberately limited role: it enforces the declared linear network physics, the tight inventory interval induced by the endpoint envelope, and non-negativity while making the smallest standardized change to the raw prediction. It neither predicts a layer shape nor enforces the nonlinear settling-flux equations. The production surrogate optimizer resolves that exact lower QP within a seven-control search and audits its primal--dual KKT conditions independently; analytical active-set sensitivities remain primary, while a deterministic value-only fallback preserves a reportable feasible incumbent when those sensitivities are unavailable. Strong KKT evidence and finite-direction, finite-resolution two-scale feasible-poll evidence are reported as distinct convergence tiers, without overstating an incomplete or budget-limited result as stationary or optimal.

The simplification concentrates statistical capacity and validation on quantities that actually enter recycle closure, water quality, the objective, and engineering limits. Its cost is equally explicit: aggregate inventory feasibility cannot establish a realizable nonlinear settling profile. Nonlinear kinetics, all ten layer balances, stability, and objective comparability are therefore adjudicated by replaying every available selected candidate on the same exact nonsmooth mechanistic model. The 3,343-row holdout and ten scenario inputs are post-selection evidence after the response redesign and support descriptive rather than confirmatory claims. Numerical conclusions will be finalized only after all required tables are repopulated by the revised run under the dated protocol above; no result from the superseded layer-output surrogate will be carried forward as evidence for this model.
