...
\shorttitle{Three-Route Optimization of Activated Sludge}
\shortauthors{Egberto F. Selerio Jr.}

\title[mode=title,size=\Large]{Three-Route Optimization of a Recycling Mixer--Reactor--Clarifier Activated-Sludge System Using Whole-System and Shared-Unit Surrogates}
...
\begin{abstract}
Activated-sludge optimization is difficult because biological conversion, internal mixed-liquor recycle, return activated sludge, wasting, and secondary clarification form one nonlinear closed loop. This work formulates and compares three optimization routes for the same plant: an existing whole-system projected surrogate ($S$), a new shared-unit projected surrogate ($U$), and an existing smooth mechanistic nonlinear program ($M$). Route $S$ uses one quadratic multiresponse model for the mixer, every reactor outlet, both Clarifier outlet-flow vectors, and one aggregate Clarifier-solids inventory. Route $U$ instead reuses one fitted CSTR map in every reactor and applies one fitted Clarifier map; an exact mixer equation closes their implicit recycle loop. Both statistical routes assemble the same $n_\chi=(N+3)F+1$ operational response and use the identical scaled-Euclidean projection to impose recycle mixing, stoichiometric-invariant transport, Clarifier component conservation, soluble pass-through, particulate densification, tight outlet-derived inventory bounds, and non-negativity. Thus the comparison changes statistical factorization, not the plant, objective, controls, projection, or mechanistic comparator. The case study uses five equal-volume reactors, independently adjustable aeration in reactors 3--5, a ten-layer mechanistic Clarifier, 16,714 accepted mechanistic design states, five-fold plant-grouped cross-validation, and ten post-selection influent scenarios. Route $U$ is assessed both one step at a time and after free-running recycle closure. Each surrogate optimizer cold-solves its required inner problems at every distinct trial; verified total derivatives may support a strong first-order KKT tier, whereas deterministic value-only optimization and two-scale feasible no-descent polls provide an explicitly finite-resolution tier when derivative conditions fail. Every selected $S$, $U$, and $M$ candidate is replayed on the same exact nonsmooth, layer-resolved aggregate-solids model. The framework separates architectural error, physical reconciliation, recycle-root reliability, decision quality, smoothing effects, and computational cost without asserting layer-profile prediction or global optimality.
\end{abstract}

\begin{highlights}
\item Whole-system and shared-unit surrogates optimize the same recycling plant.
\item One fitted CSTR map is reused across all biological reactor stages.
\item Both surrogate routes share one joint convex physical projection.
\item Recycle-root and projection sensitivities support auditable optimization.
\item All three routes are judged on one exact ten-layer reference model.
\end{highlights}

\begin{keywords}
activated sludge \sep process optimization \sep recycle \sep shared-unit surrogate \sep constrained surrogate \sep quadratic projection
\end{keywords}
...
$D_\chi,D_y,D_b,D_g,D_T$ & Positive plant-response, mechanistic-state, physical-row, and quality scales\\
$D_{y,R},D_{y,C}$ & Positive response scales for the shared reactor and Clarifier fits\\
...
$n_\chi,n_y,n_{\phi,S},n_{\phi,R},n_{\phi,C},d_z$ & Plant-response, mechanistic-state, feature, and statistical-input dimensions\\
...
$\chi_{{\rm raw},k},\widehat\chi_k$ & Raw and physically projected operational responses for surrogate route $k\in\{S,U\}$\\
$y$ & Reactor-and-Clarifier mechanistic state\\
$\nu,\rho$ & Stoichiometric matrix and process-rate vector\\
$\phi_S,\phi_R,\phi_C$ & Standardized unique-monomial feature vectors for the whole-system, reactor, and Clarifier fits\\
$\mathcal S_R,\mathcal S_C,\mathcal F_U$ & Shared reactor map, Clarifier map, and shared-unit recycle-closure residual\\
...
The present formulation gives regression and physics different, auditable roles, extending invariant-aware surrogate modeling from isolated responses to the recycling plant \citep{Selerio2026}. It compares two statistical factorizations without altering the process definition. The whole-system architecture predicts the complete decision-relevant closed-loop response in one quadratic map. The shared-unit architecture reuses one quadratic CSTR map across all reactor stages, couples it to one quadratic Clarifier map, and closes the recycle through the exact mixer equation. Internal Clarifier layers are omitted from both statistical responses because they are not needed to close the recycle, calculate outlet quality, or evaluate the operating objective; their volume-weighted total is retained because whole-plant solids retention time depends on Clarifier inventory. The same strictly convex quadratic program (QP) then finds the nearest response from either surrogate satisfying a stated set of linear physical relations. The two projected-surrogate routes and the smooth mechanistic NLP use identical controls, objective, and engineering limits. Nonlinear biological rates and the nonlinear settling law are not attributed to the projection; they are checked independently with the full layer-resolved state of the declared aggregate-solids mechanistic model. This distinction is important because satisfying conservation and aggregate inventory bounds is necessary for a credible plant response, but it is not sufficient to establish kinetic or layer-profile accuracy.
...
\subsection{Whole-system raw operational-response surrogate}

The existing whole-system surrogate, designated route $S$, predicts every quantity needed to close the recycle and evaluate the objective and operating constraints, without reproducing latent spatial detail. Both surrogate architectures use the same output stack in process order:
\begin{equation}
...
\subsection{Shared-unit raw operational-response surrogate}

The added route $U$ preserves the same plant topology but factorizes the statistical prediction by unit operation. One fitted quadratic reactor map is reused, with exactly the same coefficient matrix, in every biological stage; one fitted quadratic Clarifier map is used once:
\begin{align}
c_{i,U}^{\rm raw}&=\mathcal S_{R,\gamma_{\rm rxn}}
  (c_{i-1,U}^{\rm raw},d_i,a_i),\qquad i=1,\ldots,N,
\notag\\
\col(g_{E,U}^{\rm raw},g_{U,U}^{\rm raw},M_{{\rm cl},U}^{\rm raw})
&=\mathcal S_{C,\gamma_{\rm cl}}(c_{N,U}^{\rm raw},r_R,w),
\qquad c_{0,U}^{\rm raw}=m_U^{\rm raw}.
\label{eq:unitmaps}
\end{align}
Here $\mathcal S_{R,\gamma_{\rm rxn}}:\R^{F+2}\to\R^F$ and $\mathcal S_{C,\gamma_{\rm cl}}:\R^{F+2}\to\R^{2F+1}$ use separately standardized quadratic feature vectors $\phi_R$ and $\phi_C$, response scales $D_{y,R}$ and $D_{y,C}$, and independently selected ridge parameters. Sharing $\mathcal S_R$ assumes identical CSTR kinetics and parameterization; heterogeneous units would require declared unit descriptors or separate fits. If a variable material addition $\xi_i$ is admitted, its independent coordinates must also enter the reactor map; the present case has $\xi_i=0$.

No mixer model is fitted. Given a trial mixer state $m$, the reactor train and Clarifier are evaluated forward with Equation~\eqref{eq:unitmaps}, after which the exact mixer balance defines the $F$-dimensional recycle residual
\begin{equation}
\mathcal F_U(m;\vartheta,x)=q_Pm-x-r_Ic_{N,U}^{\rm raw}(m)
-\frac{r_R}{q_U}g_{U,U}^{\rm raw}(m)=0.
\label{eq:unitclosure}
\end{equation}
A converged, independently audited root assembles
$\chi_{{\rm raw},U}=\col(m_U^{\rm raw},c_{1,U}^{\rm raw},\ldots,
c_{N,U}^{\rm raw},g_{E,U}^{\rm raw},g_{U,U}^{\rm raw},M_{{\rm cl},U}^{\rm raw})$
in the same $n_\chi$ coordinates as route $S$. The root is solved deterministically from two target-independent starts and must pass frozen residual, agreement, and Jacobian-conditioning checks. Recycle-root uniqueness is not inferred from downstream projection uniqueness; failure of the root contract makes that route-$U$ evaluation unavailable.

\subsection{Common physical projection for both surrogate routes}
...
\label{eq:eqcount}
\end{equation}
rows: one mixer balance, $N$ invariant transitions, one full Clarifier balance, and soluble pass-through. No separate endpoint identities are needed because the endpoint TSS values are derived from the outlet flows.
...
The correction is measured in the same standardized plant-response coordinates $D_\chi$ for both architectures, so that the large-magnitude inventory coordinate does not overwhelm a small nutrient coordinate. The local fitting scales $D_{y,R}$ and $D_{y,C}$ never replace this common projection metric. For each $k\in\{S,U\}$, the projected displacement and response are
\begin{align}
\widehat u_k(\vartheta,x)=\argminop_{u\in\R^{n_\chi}}\quad&\tfrac12\|u\|_2^2
\label{eq:projection}\\
\text{subject to}\quad&
D_b^{-1}\{\mathcal H(\vartheta)[\chi_{{\rm raw},k}+D_\chi u]-\mathfrak b(\vartheta,x)\}=0,\notag\\
&\chi_{{\rm raw},k}+D_\chi u\ge0,\notag\\
&D_g^{-1}\mathcal G(\vartheta)[\chi_{{\rm raw},k}+D_\chi u]\le0,\notag\\[1mm]
\widehat\chi_k(\vartheta,x)={}&\chi_{{\rm raw},k}(\vartheta,x)+D_\chi\widehat u_k(\vartheta,x).
\label{eq:projected}
\end{align}
$D_b$ and $D_g$ improve numerical conditioning but do not alter the feasible set. The projection is applied once after the complete raw plant response is assembled; per-unit projections would change the method and are not used. Whenever that set is nonempty, the identity Hessian makes the objective strictly convex and coercive, so the projected response exists and is unique for an available raw response. In the no-addition case, nonemptiness holds at every declared design point by construction: set $m=c_i=x$ for all $i$, $g_E=q_Ex$, $g_U=q_Ux$, and $M_{\rm cl}=V_{\rm cl}t_X^\top x$. For nonzero additions, nonemptiness is instead a required domain-admissibility check because the invariant right-hand sides change.
...
\begin{equation}
\|D_\chi^{-1}(\widehat\chi_k-\chi_{\rm ref})\|_2^2
\le \|D_\chi^{-1}(\chi_{{\rm raw},k}-\chi_{\rm ref})\|_2^2
-\|\widehat u_k\|_2^2,\qquad k\in\{S,U\}.
\label{eq:nonexpansive}
\end{equation}
Thus the physical correction cannot increase standardized squared error to a reduced mechanistic reference point that is actually in the projection set. A numerically near-feasible target requires the finite-distance bound introduced in the methodology; small infeasibility is not treated as exact feasibility. Neither result reconstructs an internal Clarifier profile or proves that the projected response satisfies all kinetic or nonlinear settling equations, which are tested separately by full mechanistic replay.
...
Physical feasibility alone does not prevent optimization from asking the projection to make an unusually large repair or from exploiting a state that conserves inventories but is kinetically implausible. The existing route-$S$ vector $g_{{\rm trust},S}$ retains its four development-supported diagnostics: standardized correction $\kappa_{{\rm corr},S}=\|\widehat u_S\|_2/\sqrt{n_\chi}$; whole-system regularized feature leverage; deviation from a common particulate recovery; and the scaled smooth reactor-balance residual. Route $U$ has a separately calibrated $g_{{\rm trust},U}$ containing only its route-specific correction, particulate-recovery, smooth reactor-balance, $N$ shared-reactor-leverage, and Clarifier-leverage constraints. Separate hard checks on recycle-root residual, two-start agreement, and Jacobian conditioning determine whether a route-$U$ value exists; they are evaluation prerequisites outside $g_{{\rm trust},U}$, not differentiable upper inequalities with KKT multipliers. Route $U$'s local leverage statistics cannot be replaced by route $S$'s whole-system leverage. Explicit formulas and thresholds are provided in the Supplementary Material. Each threshold is determined from grouped development calculations without using the post-selection holdout or optimization responses. No scalar aggregate can reproduce the discarded layer-flux residual, so nonlinear Clarifier plausibility is deliberately reserved for the exact $L$-layer replay rather than represented by a surrogate trust statistic.

The existing whole-system surrogate operational problem is
\begin{equation}
\begin{aligned}
...
The added shared-unit route uses the same outer engineering problem but its own statistical-support constraints and its audited inner recycle root:
\begin{equation}
\begin{aligned}
\min_{\vartheta}\quad&J[\vartheta,\widehat\chi_U(\vartheta,x);\varpi]\\
\text{subject to}\quad&\vartheta\in\mathcal V,\\
&\chi_{{\rm raw},U}(\vartheta,x)\text{ is assembled from an accepted root of Equation~\eqref{eq:unitclosure}},\\
&g_{\rm eng}[\vartheta,\widehat\chi_U(\vartheta,x)]\le0,\\
&g_{{\rm trust},U}[\vartheta,\chi_{{\rm raw},U},\widehat\chi_U]\le0,\\
&\widehat\chi_U(\vartheta,x)\text{ solves Equation~\eqref{eq:projection}}.
\end{aligned}
\label{eq:unitbilevel}
\end{equation}
Its selected feasible incumbent is denoted $\vartheta_U$. The closure root is not declared unique; only a root satisfying the frozen two-start agreement and numerical audits is eligible for projection and optimization.
...
Equation~\eqref{eq:gap} remains the independent lower-QP optimality identity, but it is not embedded as a constraint in either production surrogate problem. The existing route-$S$ calculation continues to operate directly in the $d_\vartheta=7$ normalized controls and cold-solves Equation~\eqref{eq:standardqp} with a convex-QP method at every distinct trial control \citep{Stellato2020}. On a stable active set, total derivatives of the projected response and every upper function are obtained by differentiating the equality-plus-active-inequality QP KKT system. If that derivative construction is unavailable, deterministic value-only COBYQA evaluates the same upper problem, with a new cold solve of the unchanged exact QP at each distinct fallback trial. Every upper constraint is divided by a fixed positive, development-frozen scale, so feasibility is unchanged and no row scale depends on the current iterate. The calculation uses the deterministic box center as its only start and cold-replays its selected endpoint independently.

Route $U$ additionally requires a valid total derivative through its implicit recycle root. When the closure Jacobian is nonsingular and passes the frozen conditioning audit,
\begin{equation}
\frac{d m_U^{\rm raw}}{d\vartheta}
=-\left(\frac{\partial\mathcal F_U}{\partial m}\right)^{-1}
\frac{\partial\mathcal F_U}{\partial\vartheta},
\label{eq:unitrootsensitivity}
\end{equation}
and differentiation of Equation~\eqref{eq:unitmaps} completes $d\chi_{{\rm raw},U}/d\vartheta$ before the same active-set projection sensitivity is applied. A route-$U$ strong-KKT classification requires independent finite-difference agreement for this complete root-plus-projection derivative chain as well as all route-$S$-style upper and lower audits. If any root or projection derivative condition fails, route $U$ uses the same deterministic value-only fallback and finite-resolution poll logic, but each distinct value evaluation first cold-solves and audits the recycle root and then cold-solves the projection QP.
...
The third route, $M$, removes both regression and projection. Its state contains the concentrations that actually accumulate in the reduced model:
\begin{equation}
...
For this case, the whole-system route has $d_z=d_\vartheta+F=27$ and $n_{\phi,S}=406$. Each shared-unit input has $F+2=22$ coordinates, so $n_{\phi,R}=n_{\phi,C}=276$; the reactor and Clarifier coefficient matrices have dimensions $20\times276$ and $41\times276$, respectively. Both surrogate routes assemble $n_\chi=161$ responses and share $n_h=75$, $n_g=12$, and $n_q=173$, while the mechanistic route has $n_y=110$. These are evaluations of Equations~\eqref{eq:state}, \eqref{eq:featurecount}, \eqref{eq:eqcount}, and \eqref{eq:G}; they are not fixed architectural constants. Changing $N$ or the free-control set changes route $S$ and the assembled route-$U$ problem and requires renewed validation. The shared reactor coefficients need not change with $N$ only when reactor kinetics, parameterization, and local input support remain identical. Changing $L$ changes $n_y$, the mechanistic discretization, and the deterministic construction of $M_{\rm cl}$, but not $n_\chi$; it still requires renewed mechanistic and response validation.
...
\item construct the common reduced plant response, pooled reactor transitions, and Clarifier targets from the stored states; select route $S$ and route $U$ ridge settings by plant-grouped five-fold cross-validation entirely within the 13,371 development states; and fit the final whole-system, shared-reactor, and Clarifier models;
\item evaluate both surrogate architectures once on the 3,343 post-selection holdout states for descriptive assessment, including route $U$'s teacher-forced unit diagnostics and independently initialized free-running recycle closure before the common projection;
\item attempt the nominal and ten influent scenarios by Equations~\eqref{eq:bilevel}, \eqref{eq:unitbilevel}, and \eqref{eq:mechanisticnlp}; certify each surrogate by its applicable tier; preserve the existing single route-$S$ conditional recovery for a failed primary direct solve; and retain every success or failure status; and
...
Later on 25 August 2026, explicit author instruction added the shared-unit projected surrogate as a third optimization route; it did not replace or revise the whole-system surrogate or smooth mechanistic NLP. This amendment followed inspection of earlier surrogate outputs and is therefore outcome-aware. The frozen mechanistic states can supply the new reactor-transition and Clarifier training targets, but no route-$U$ fit, cross-validation result, closure audit, optimization, replay, or timing is inherited. All five reactor transitions from a plant state retain that state's fold membership. The existing post-selection holdout and ten scenario inputs remain descriptive for route $U$, and any numerical claim comparing three routes requires the new route to pass its own frozen admission, closure, optimization, and exact-replay contracts.
...
For route $U$, each plant row contributes $N=5$ ordered reactor transitions, giving 66,855 development transition rows, and one Clarifier row, giving 13,371 Clarifier rows. The $N$ transitions from a plant state always remain together in that plant's fold, preventing the same trajectory from appearing in both fit and validation subsets. The reactor and Clarifier coefficient matrices use the same ridge form as Equation~\eqref{eq:ridge}, with separate local scales and penalties $\gamma_{\rm rxn}$ and $\gamma_{\rm cl}$. Each penalty is selected independently by the same one-standard-error rule applied to its teacher-forced standardized validation error; pooled and stagewise reactor scores and the Clarifier score are retained. The selected pair is then subjected to a separate, mandatory free-running out-of-fold closure and assembled-response assessment. No target state initializes or selects a closure root, and local scores cannot substitute for this plant-level gate. Final $\mathcal S_{R,\gamma_{\rm rxn}}$ and $\mathcal S_{C,\gamma_{\rm cl}}$ fits use all development rows after selection.

For each surrogate route, the frozen post-selection holdout reports descriptive RMSE, MAE, bias, normalized RMSE, and $R^2$ for the mixer, each reactor, both outlet-flow blocks, Clarifier solids inventory, and the full reduced response. Route $U$ additionally reports teacher-forced unit metrics, free-running closure success and two-start agreement, and error growth along the reactor train. Raw and projected assembled predictions are reported side by side. No target state initializes or selects a validation root. For holdout row $i$, let $\mathcal C_i$ be its projection polyhedron and let $P_{\mathcal C_i}$ be metric projection in the norm $\|v\|_{D_\chi^{-1}}:=\|D_\chi^{-1}v\|_2$. Because a numerical mechanistic target $\chi_i$ satisfies equations only to finite tolerance, its feasibility distance $\delta_{{\rm feas},i}=\|P_{\mathcal C_i}(\chi_i)-\chi_i\|_{D_\chi^{-1}}$ is reported. The following general bound is checked for every target and either surrogate architecture:
\begin{equation}
\|P_{\mathcal C_i}(\chi_{{\rm raw},k,i})-\chi_i\|_{D_\chi^{-1}}
\le \|\chi_{{\rm raw},k,i}-\chi_i\|_{D_\chi^{-1}}+\delta_{{\rm feas},i},
\qquad k\in\{S,U\}.
\label{eq:finitefeasibility}
\end{equation}
Equation~\eqref{eq:nonexpansive} is invoked only for a target known to be exactly feasible; a merely small positive $\delta_{{\rm feas},i}$ does not justify the stronger squared inequality. Route $S$ retains its existing admission gates. Route $U$ has separate gates requiring all development out-of-fold closures and QPs to pass their root and primal--dual audits, full assembled-response and inventory-coordinate raw nRMSE values below one, and successful freezing of every route-$U$ trust limit. The separate inventory criterion prevents its single coordinate from being hidden in the 161-coordinate average even though it enters SRT directly. Holdout metrics are descriptive and do not alter either route's gates. Failure of one surrogate route is persisted without reclassifying the other; a complete three-route comparison is eligible only where all applicable route and replay checks pass, while pairwise eligibility is assessed independently. A failed scientific gate does not trigger refitting. A non-finite or incomplete numerical object from which a downstream problem cannot be defined remains a hard computational failure.

\subsection{Nominal optimization and exact common-reference replay}

The nominal influent is the midpoint of each declared influent interval. All three optimization routes use the same free controls, bounds, objective priorities, and engineering constraints. Each route begins at the componentwise center of the normalized seven-dimensional box. For the existing route $S$, the former primal--dual-gap IPOPT continuation is not executed; its exact-QP active-set problem in the seven normalized controls remains the primary solver.

The original projection QP is cold-solved to its acceptance tolerances at every distinct route-$S$ trial point. On a stable active set, response derivatives follow from the equality-plus-active-inequality QP KKT sensitivity system after rank and conditioning checks; an active-set change triggers a fresh QP and derivative reconstruction. If exact active-set derivatives cannot be constructed, deterministic value-only COBYQA continues within the normalized control bounds. Every distinct fallback trial receives a new cold solve and complete audit of the same projection QP; the fallback introduces neither an approximate projection nor finite-difference derivatives. Its selected best feasible visited point is cold-projected again. This is the existing route-$S$ algorithm and is not changed by adding route $U$.

At every distinct route-$U$ trial, two deterministic cold recycle roots are attempted and audited before the same projection QP is cold-solved. Exact duplicate controls alone may reuse the complete cached evaluation; no root or QP warm start crosses distinct controls. Analytical optimization is allowed only when the implicit-root and projection derivative chain passes the declared rank, conditioning, active-set, and finite-difference audits. Otherwise, the same deterministic value-only COBYQA logic is used. For either surrogate route, a candidate receives the strong first-order tier only if its applicable derivative chain and independently recomputed lower-QP and upper KKT audits pass. Otherwise, complete feasible no-descent polls at both normalized radii $10^{-3}$ and $10^{-4}$ can qualify it only at finite resolution under protocol v3. After a winning poll step, up to 16 search-only complete route evaluations expand geometrically along that ray; a new complete poll is mandatory after every accepted move, so an acceleration trial never supplies certification evidence. Each completed poll must contain a feasible nonzero displacement; direction rank is diagnostic only. Any accepted fine-scale move forces the coarse poll to be repeated before certification, and each route has its own 10,000-evaluation certification safety budget. A failed or incomplete poll leaves stationarity unresolved. First-order stationarity is not a certificate of local minimality, and the finite poll is neither KKT/Clarke stationarity nor proof of local or global optimality.

The primary direct mechanistic solve retains its three-stage smoothing continuation from the center and its full-space endpoint audit. To leave route $M$ unchanged, if that primary attempt fails, at most one recovery may be run from the same case's certified route-$S$ control, using the unchanged three-stage sequence. Route $U$ never supplies an additional recovery start. No recovery is run after a successful primary attempt or when the route-$S$ candidate is uncertified. Thus recovery remains a disclosed dependency on the original surrogate route, not an additional independent basin search. A validated feasible but budget-limited or stationarity-unresolved point remains a reportable incumbent. If a route produces no validated candidate, that failure remains in the case denominator and execution proceeds.

For each available selected candidate $k\in\{S,U,M\}$, an exact nonsmooth mechanistic replay produces a full layer state that is mapped to the common reduced response $\chi_{{\rm ref},k}$ by Equation~\eqref{eq:state} with Equation~\eqref{eq:inventory}. The optimizer-native responses are $\chi_{{\rm nat},S}=\widehat\chi_S$, $\chi_{{\rm nat},U}=\widehat\chi_U$, and the reduced mapping of the smooth state, $\chi_{{\rm nat},M}=\chi_{{\rm sm},M}$. Both surrogate architectures are also evaluated, wherever their inner problems succeed, at all three selected controls so prediction error is not confounded with decision difference. For $p\in\{S,U\}$ and $k\in\{S,U,M\}$, define
\begin{align}
e_{{\rm raw},p|k}&=D_\chi^{-1}[\chi_{{\rm raw},p}(\vartheta_k,x)-\chi_{{\rm ref},k}],\notag\\
e_{{\rm proj},p|k}&=D_\chi^{-1}[\widehat\chi_p(\vartheta_k,x)-\chi_{{\rm ref},k}],\notag\\
...
\Delta J_{S-U}&=J(\vartheta_S,\chi_{{\rm ref},S})-J(\vartheta_U,\chi_{{\rm ref},U}),\notag\\
\Delta J_{S-M}&=J(\vartheta_S,\chi_{{\rm ref},S})-J(\vartheta_M,\chi_{{\rm ref},M}),\notag\\
\Delta J_{U-M}&=J(\vartheta_U,\chi_{{\rm ref},U})-J(\vartheta_M,\chi_{{\rm ref},M}).
\label{eq:comparisons}
\end{align}
Each valid, physically audited exact replay retains its exact objective and objective components even when the exact response is engineering-infeasible. Such a candidate is reported but cannot enter a paired route comparison. Each difference in Equation~\eqref{eq:comparisons} is independently eligible only when that pair's two candidates have valid, engineering-feasible exact replays; failure of the third route does not suppress an otherwise valid pair. These quantities compare selected feasible incumbents and are not called regret because no route supplies a global certificate. A symmetric relative objective difference and normalized control RMS and maximum differences accompany every eligible pair.
...
Ten influent vectors drawn previously by an independent 20-dimensional Latin hypercube over the stated influent ranges are frozen as post-selection stress cases. Their old layer-output-model responses and decisions are excluded, and no revised response or decision enters fitting or tuning. All three optimization routes are run and adjudicated for each case; reference replay is attempted wherever a route supplies a selected decision. The analysis reports each route's status and, where available, all seven selected controls; stagewise dissolved oxygen, TN, TP, and TSS; Clarifier outlet composites and $M_{\rm cl}$ for every shared response; mechanistic layer profiles where defined; SRT, SOR, and SLR; objective components; route-specific projection correction; route-$U$ closure diagnostics; and solve time. Ten post-selection cases support descriptive casewise comparisons, not independent or population-level reliability claims.

\subsection{Timing and numerical acceptance}

Timing separates route-$S$ regression, route-$U$ reactor and Clarifier evaluations, route-$U$ recycle-root work, the common projection QP, each complete surrogate optimization and convergence certification, primary smooth mechanistic optimization, any conditional direct recovery, and exact nonsmooth replay. The same processor allocation and thread count are used for all three optimization routes. Setup and solve times, case attempts and outcomes, root and QP evaluations, iterations, active-set/fallback method, COBYQA evaluation-budget status, poll direction set, feasible-displacement count and rank diagnostic, accepted moves, coarse revalidations, certification budget status and tier, smoothing stages, recovery status, and exact replays are all reported. A solver success flag is never sufficient: the applicable scaled primal, dual, stationarity, complementarity, root-agreement, physical-balance, reduced-stability, domain, engineering-feasibility, and branch-qualification checks must also be reported under the Supplementary Material contract.

\section{Results and discussion}

No numerical result from the superseded 170-coordinate surrogate is inserted, and no result for the newly specified shared-unit route is inferred before it is run. Results are populated only after both 161-coordinate surrogate architectures have been fitted, plant-group cross-validated, projected, and optimized under their revised checks, and after all three routes have been attempted in all eleven post-selection optimization cases. Generation reporting uses all 18,211 completed attempts as its denominator and shows how 13,371 development and 3,343 post-selection holdout rows were selected from the accepted checkpoints; downstream prediction denominators use only the applicable accepted rows. Failed or unresolved optimization and replay cases remain visible in their case denominators and result tables rather than being replaced or omitted. Tables~\ref{tab:generation_results}--\ref{tab:timing_results} define the minimum revised result set and prevent selective reporting after outcomes are known.
...
\begin{table}[htbp]
\centering
\caption{Plant-grouped five-fold selection and descriptive post-selection holdout performance. Raw and projected errors are required for every assembled reduced-response block and both surrogate routes.}
...
$U$ & Reactors 1--$N$ (each reported) & -- & -- & -- & -- & --\\
$S$ & Clarifier overflow & -- & -- & -- & -- & --\\
$U$ & Clarifier overflow & -- & -- & -- & -- & --\\
$S$ & Clarifier underflow & -- & -- & -- & -- & --\\
$U$ & Clarifier underflow & -- & -- & -- & -- & --\\
$S$ & Clarifier solids inventory $M_{\rm cl}$ & -- & -- & -- & -- & --\\
$U$ & Clarifier solids inventory $M_{\rm cl}$ & -- & -- & -- & -- & --\\
$S$ & Full reduced response & -- & -- & -- & -- & --\\
$U$ & Full reduced response & -- & -- & -- & -- & --\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Required shared-unit diagnostic decomposition. Teacher-forced module scores diagnose the two fits; only the independently initialized free-running rows assess the assembled route-$U$ plant response.}
\label{tab:unit_prediction_results}
\small
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrr}
\toprule
Diagnostic & CV nRMSE & Holdout nRMSE & Successful roots / rows & Agreement passes / roots\\
\midrule
Reactor, pooled one step & -- & -- & n.a. & n.a.\\
Reactor, stages 1--5 separately & -- & -- & n.a. & n.a.\\
Clarifier, one step & -- & -- & n.a. & n.a.\\
Complete free-running raw response & -- & -- & -- & --\\
Complete free-running projected response & -- & -- & -- & --\\
\bottomrule
\end{tabular}
}
\end{table}
...
$U$ & Projection correction $\kappa_{{\rm corr},U}$ & -- & -- & --\\
$S$ & Whole-system leverage $\ell_S$ & -- & -- & --\\
$U$ & Reactor leverage $\ell_{U,R,i}$, maximum over stages & -- & -- & --\\
$U$ & Clarifier leverage $\ell_{U,C}$ & -- & -- & --\\
$S$ & Particulate split $\kappa_{{\rm split},S}$ & -- & -- & --\\
$U$ & Particulate split $\kappa_{{\rm split},U}$ & -- & -- & --\\
$S$ & Reactor residual $\kappa_{{\rm rxn},S}$ & -- & -- & --\\
$U$ & Reactor residual $\kappa_{{\rm rxn},U}$ & -- & -- & --\\
$U$ & Recycle residual/agreement/conditioning & -- & -- & --\\
\bottomrule
\end{tabular}
}
\end{table}
...
\midrule
Route-$S$ raw reduced & Post-selection holdout and selected decisions & -- & -- & -- & -- & --\\
Route-$S$ projected reduced & Post-selection holdout and selected decisions & -- & -- & -- & -- & --\\
Route-$U$ raw reduced & Post-selection holdout and selected decisions & -- & -- & -- & -- & --\\
Route-$U$ projected reduced & Post-selection holdout and selected decisions & -- & -- & -- & -- & --\\
...
\begin{table}[htbp]
\centering
\caption{Required scope-specific feasibility and nonlinear audit. Each entry reports the applicable maximum residual and exceedance count; n.a. means that the source has no layer state on which to evaluate that quantity.}
\label{tab:scope_audit_results}
\scriptsize
\begin{tabular}{lccccc}
\toprule
Source & Inventory bounds & Reactor residual & Layer envelope & Layer residual & Stability\\
\midrule
Route-$S$ raw reduced & -- & -- & n.a. & n.a. & n.a.\\
Route-$S$ projected reduced & -- & -- & n.a. & n.a. & n.a.\\
Route-$U$ raw reduced & -- & -- & n.a. & n.a. & n.a.\\
Route-$U$ projected reduced & -- & -- & n.a. & n.a. & n.a.\\
Smooth-direct native & -- & -- & -- & -- & --\\
Exact mechanistic & -- & -- & -- & -- & --\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lrrr}
\toprule
Quantity & Whole-system $S$ & Shared-unit $U$ & Mechanistic $M$\\
\midrule
$H,a_3,a_4,a_5,r_I,r_R,w$ & -- & -- & --\\
Route-$S$ native objective & -- & n.a. & n.a.\\
Route-$U$ native objective & n.a. & -- & n.a.\\
Route-$M$ native objective & n.a. & n.a. & --\\
...
\begin{table}[htbp]
\centering
\caption{Required nominal response and mechanistic-profile comparison. The final table contains separate panels at $\vartheta_S$, $\vartheta_U$, and $\vartheta_M$. Both surrogate maps are evaluated wherever defined at every selected control; layer rows exist only for smooth-direct and exact mechanistic states.}
\label{tab:profile_results}
\scriptsize
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrrrr}
\toprule
Location and reported quantity & $S$ raw & $S$ projected & $U$ raw & $U$ projected & Native $k$ & Exact\\
\midrule
Mixer: COD, TN, TP, TSS & -- & -- & -- & -- & -- & --\\
...
Mechanistic layer $\ell=1,\ldots,L$: TSS & n.a. & n.a. & n.a. & n.a. & $k=M$ only & --\\
\bottomrule
\end{tabular}
}
\end{table}

The nominal discussion will trace raw, projected, optimizer-native, and exact shared responses from the mixer through the reactors, outlet streams, and $M_{\rm cl}$. Detailed Clarifier layers will be discussed only for smooth-direct and exact mechanistic states. The discussion will identify which inventories required correction, which controls or engineering constraints were active, whether any replay carries a branch-boundary qualifier, and whether the three exact-reference objectives and their pairwise differences differ materially.

\subsection{Casewise exact common-reference validation}

\begin{table}[htbp]
\centering
\caption{Exact nonsmooth common-reference adjudication of every available selected candidate (at most 33 decisions and 66 exact starts). Counts use the actual candidate denominator; the post-selection holdout block is not part of this validation.}
\label{tab:common_reference_results}
\begin{tabular}{lrrrr}
\toprule
Adjudication count & Route $S$ & Route $U$ & Route $M$ & Total\\
\midrule
...
Exact engineering-feasible replays & -- & -- & -- & --\\
Eligible for at least one pairwise comparison & -- & -- & -- & --\\
Eligible $S$--$U$ pairs & \multicolumn{4}{c}{--}\\
Eligible $S$--$M$ pairs & \multicolumn{4}{c}{--}\\
Eligible $U$--$M$ pairs & \multicolumn{4}{c}{--}\\
All three jointly eligible & \multicolumn{4}{c}{--}\\
\bottomrule
\end{tabular}
\end{table}

This table does not claim whole-domain smooth/nonsmooth equivalence. Branch-boundary-qualified rows remain eligible when the required two-start exact replay, physical and stability audits, and engineering checks pass. A valid exact objective remains reported if the engineering check fails, but its pairwise-comparison eligibility is false. Separate counts are reported for eligible $S$--$U$, $S$--$M$, and $U$--$M$ pairs and for cases in which all three candidates are jointly eligible. Candidate absence, route failure, invalid replay, and comparison ineligibility are retained separately rather than collapsed into a favorable denominator.

\subsection{Influent scenarios and optimization comparison}

\begin{table}[htbp]
\centering
\caption{Required post-selection influent-scenario comparison. The full table contains ten rows. Every objective and signed difference is calculated on the exact common reference and shown only for the corresponding eligible route or pair. Route-specific response errors and the ratios $t_U/t_S$, $t_M/t_S$, and $t_M/t_U$ accompany this objective panel in the archived table.}
\label{tab:sensitivity_results}
\small
\begin{tabular}{lrrrrrr}
\toprule
Case & $J_{S,\rm ref}$ & $J_{U,\rm ref}$ & $J_{M,\rm ref}$ & $\Delta J_{S-U}$ & $\Delta J_{S-M}$ & $\Delta J_{U-M}$\\
\midrule
1--10 & -- & -- & -- & -- & -- & --\\
\bottomrule
\end{tabular}
...
\begin{table}[htbp]
\centering
\caption{Required casewise water-quality comparison. At all three selected decisions, response rows are raw, projected, optimizer-native, and exact mechanistic wherever defined.}
...
\midrule
1--10 & $\vartheta_S,\vartheta_U,\vartheta_M$ & raw/projected/optimizer-native/exact & -- & -- & -- & -- & --\\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
Case & Route-$S$ outcome/tier & Route-$U$ outcome/tier/root & Route-$M$ outcome/recovery & Selected $n/3$ & Reference/pair eligibility\\
\midrule
Nominal & -- & -- & -- & -- & --\\
...
\begin{table}[htbp]
\centering
\caption{Timing and solver workload for the three optimization routes. Route-$U$ totals include both cold recycle-root attempts and the common projection QP.}
\label{tab:timing_results}
\scriptsize
\begin{tabular}{lrrr}
\toprule
Metric & Whole-system $S$ & Shared-unit $U$ & Mechanistic $M$\\
\midrule
...
Median cold recycle-root attempts / case & n.a. & -- & n.a.\\
...
Conditional recovery attempts / primary failures & n.a. & n.a. & --\\
...
The formulation now defines three optimization routes for the same $N$-stage recycling mixer--reactor--Clarifier plant. Route $S$ retains one whole-system surrogate, route $U$ reuses one fitted CSTR map across every reactor and couples it to one Clarifier map through an exact mixer recycle closure, and route $M$ retains the smooth mechanistic NLP. Both statistical architectures assemble the same reduced operational response. Outlet component flows determine the effluent and underflow TSS endpoints, while the one retained Clarifier scalar, $M_{\rm cl}$, supplies the storage term needed for whole-plant SRT. Neither surrogate predicts internal layer profiles, and the common statistical response dimension remains independent of the number of mechanistic Clarifier layers.

The common projection has a deliberately limited role: after either raw surrogate has assembled all 161 coordinates, it enforces the same declared linear network physics, tight endpoint-induced inventory interval, and non-negativity while making the smallest standardized change. It neither closes route $U$'s nonlinear learned recycle loop nor enforces nonlinear kinetics or settling fluxes. Route $S$ retains its existing exact-QP optimizer. Route $U$ adds a separately audited recycle root and requires verified implicit-root plus active-projection sensitivities before it can receive a strong KKT classification; otherwise its deterministic value-only and feasible-poll tiers remain explicitly finite resolution. Projection uniqueness is not presented as evidence that the learned recycle root is unique.

The comparison isolates the effect of surrogate architecture while holding controls, engineering requirements, physical projection, and exact reference fixed. Plant-grouped validation prevents the five transitions from one plant state leaking across route-$U$ folds, and free-running closure metrics prevent favorable teacher-forced unit scores from standing in for whole-loop behavior. Nonlinear kinetics, all ten layer balances, stability, and objective comparability are adjudicated by replaying every available selected candidate from all three routes on the same exact nonsmooth mechanistic model. The 3,343-row holdout, the added route-$U$ architecture, and the ten scenario inputs are post-selection evidence and support descriptive rather than confirmatory claims. Numerical conclusions about accuracy, speed, robustness, viability, or decision quality will be finalized only after every required table is repopulated; the present revision defines an implementable, auditable comparison protocol without prejudging its outcome.
...
The study record will contain the 16,714 accepted full mechanistic states frozen from 18,211 completed attempts; the deterministic mapping from every layer state to the reduced response and $M_{\rm cl}$; the complete accepted and rejected candidate-attempt ledger; immutable plant-to-partition and grouped-fold membership; both fitted surrogate packages, local and plant-response scales, and ridge selections; route-$U$ recycle-root and derivative audits; route-indexed descriptive raw and projected post-selection-holdout predictions; every rerun nominal and influent-scenario optimization record and convergence tier; unchanged route-$S$ conditional direct-recovery provenance; casewise exact common-reference states; raw, projected, optimizer-native, and exact physical-audit ledgers; timing summaries; and the separately archived, explicitly superseded layer-output artifacts.
