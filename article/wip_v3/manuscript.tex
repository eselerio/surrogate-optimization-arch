...
\item Its invariant-constraint projection is extended system-wide.
...
The extended-ICSOR formulation retains the original whole-system second-order regression and changes only the scope of the invariant-constraint projection: the projection now acts on the complete closed-loop response. The frozen post-selection holdout and ten scenario inputs remain descriptive rather than confirmatory evidence.
...
The frozen post-selection holdout reports descriptive RMSE, MAE, bias, normalized RMSE, and $R^2$ for the extended-ICSOR mixer, each reactor, both outlet-flow blocks, Clarifier solids inventory, and full reduced response. Raw and projected assembled predictions are reported side by side. For holdout row $i$, let $\mathcal C_i$ be its projection polyhedron and let $P_{\mathcal C_i}$ be metric projection in the norm $\|v\|_{D_\chi^{-1}}:=\|D_\chi^{-1}v\|_2$. Because a numerical mechanistic target $\chi_i$ satisfies equations only to finite tolerance, its feasibility distance $\delta_{{\rm feas},i}=\|P_{\mathcal C_i}(\chi_i)-\chi_i\|_{D_\chi^{-1}}$ is reported. The following general bound is checked for every target:
\begin{equation}
...
The system-wide projection has a deliberately limited role: after extended ICSOR has assembled all 161 coordinates, it enforces the declared linear network physics, tight endpoint-induced inventory interval, and non-negativity while making the smallest standardized change. It does not enforce nonlinear kinetics or settling fluxes. Extended ICSOR retains its exact-QP optimizer and its active-projection sensitivity audit; when that derivative audit is unavailable, its deterministic value-only and feasible-poll tiers remain explicitly finite resolution.
