...
\begin{abstract}
Activated-sludge optimization is difficult because biological conversion, internal mixed-liquor recycle, return activated sludge, wasting, and secondary clarification form one nonlinear closed loop. This work formulates the system for an arbitrary number $N$ of completely mixed reactors and an arbitrary number $L$ of Clarifier layers. The freely varied coordinates may include stage residence times, independent stage aeration settings, recycle ratios, wasting, and other explicitly balanced stage inputs. A quadratic multiresponse surrogate predicts the mixer, every reactor outlet, both Clarifier outlet component-flow vectors, and the layer-solids profile. Its $n_\chi=(N+3)F+L$ responses are reconciled by a unique scaled-Euclidean projection that imposes recycle mixing, stoichiometric-invariant transport, Clarifier component conservation, soluble pass-through, particulate densification, endpoint consistency, an admissible layer envelope, and non-negativity. Operational optimization is constrained by this projected state. A seven-variable local optimizer cold-solves the exact projection quadratic program at each distinct trial control and primarily obtains total response derivatives from a verified active-set KKT sensitivity system. When that derivative system is unavailable, deterministic value-only optimization continues over the unchanged QP. Surrogate convergence is qualified either by a strong first-order KKT certificate or by complete two-scale feasible no-descent polls that make no classical-stationarity claim. The case study uses five equal-volume reactors, independently adjustable aeration in reactors 3--5, a ten-layer Clarifier, 5,000 accepted mechanistic design states, five-fold cross-validation, and ten influent scenarios. Every selected surrogate and direct candidate is replayed on the same exact nonsmooth mechanistic model; raw, projected, optimizer-native, and exact responses are audited for mass conservation and non-negativity. The formulation separates regression error, physical reconciliation, decision quality, smoothing effects, and computational cost without asserting global optimality.
\end{abstract}
...
\item Surrogate convergence has strong-KKT and finite-resolution poll tiers.
\item Every selected decision is judged on one exact nonsmooth reference model.
\end{highlights}
...
\label{eq:bilevel}
\end{equation}
Here $\mathcal V$ contains the declared bounds and any stage-allocation equalities. The lower response is unique; the upper problem remains generally nonconvex and piecewise smooth. Accordingly, $\vartheta_S$ denotes the selected feasible incumbent together with its explicitly reported convergence tier, rather than a certified global minimizer.
...
\item attempt the nominal and ten influent scenarios by both Equations~\eqref{eq:bilevel} and \eqref{eq:mechanisticnlp}, certify surrogate convergence by its declared tier, allow at most one conditional direct recovery after a primary direct failure, and retain every success or failure status; and
...
After the resumed production optimizer had demonstrated that the approximately 450-variable embedded-KKT surrogate NLP and its seven continuation stages were impractically slow, a further explicit user authorization on 23 August 2026 retired that production route and the nine-start basin search. The same run now uses one deterministic box-center start for each optimization route and each of the nominal plus ten robustness cases. The surrogate route uses the seven-variable exact-QP active-set calculation directly, with no embedded-KKT IPOPT stage or gap continuation; when exact active-set derivatives are unavailable, a deterministic value-only COBYQA fallback evaluates the unchanged cold projection QP. The direct mechanistic route retains its distinct three-stage smoothing continuation. This outcome-aware computational revision accepts the limitation to one local basin. It reuses the verified generation, fit, and assessment artifacts, but does not reuse a partial optimization result produced under the retired protocol. Every case failure, budget-limited incumbent, and unresolved audit remains reportable and does not prevent later cases or downstream replay, physical accounting, timing, and reporting from being attempted.

On 24 August 2026, after preliminary selected-case artifacts had exposed unresolved surrogate stationarity and branch-boundary sensitivity in the former validation logic, the scientific comparison protocol was revised explicitly and outcome-aware. Whole-test smooth/reference equivalence was retired as an admission test; any partial legacy rows are retained in the record but excluded from analysis. The untouched test block remains reserved for raw and projected surrogate prediction assessment. Each available selected surrogate or direct candidate is instead replayed casewise from both declared starts on one common exact nonsmooth mechanistic model. A branch-boundary ambiguity is reported as a qualifier and does not by itself reject an otherwise physically valid and reproducible reference result. Surrogate convergence now has the strong-KKT and finite-resolution two-scale feasible-poll tiers described above. The primary direct solve remains the box-center, three-stage continuation; only if it fails may one recovery be attempted, and only from that case's certified surrogate control, using the unchanged continuation. This recovery dependency is reported and is not presented as independent basin exploration. The revision reuses verified upstream and valid case artifacts, and audits mass conservation and non-negativity separately for raw, projected, optimizer-native, and every exact-replay response.

Preliminary execution of feasible-poll protocol v1 then left all eleven surrogate cases unresolved: three exhausted its 120-evaluation budget and eight were labeled rank-inconclusive. These outputs are archived as superseded diagnostics and excluded from article analysis. Their pattern exposed two defects in the evidential rule rather than eleven demonstrated optimization failures: the fine poll had limited tangent-direction coverage, and full rank in $\mathbb R^7$ was required even where active constraints restrict the local feasible cone. An explicit outcome-aware v2 refinement therefore expanded the fine poll to 106 directions, raised the shared exact-QP budget to 2,500, required at least one feasible nonzero displacement in each complete poll, treated feasible-direction rank as diagnostic only, and revalidated the coarse radius after any accepted fine-scale move. During that run, robustness case 05 accepted 36 successive fine-scale moves but reached 2,500 exact-QP evaluations before completing its next sweep. This demonstrated inefficient fixed-step search rather than a QP or feasibility failure. Protocol v3 therefore adds deterministic exact-QP ray acceleration after a winning coupled direction while leaving the final two-scale no-descent test unchanged. Both predecessor result sets are archived and excluded. The corrected rerun is placed in a new self-contained result directory containing byte-verified copies of the accepted 5,000-row data, model, assessment, and primary-search artifacts plus an explicit provenance manifest; the source run is not overwritten. The v3 result remains finite-direction and finite-resolution evidence and is not described as KKT, Clarke-stationary, locally optimal, or globally optimal.

After all eleven casewise calculations were complete, the repeated untouched-test inference benchmark was explicitly retired on 24 August 2026 before it finished. It is excluded from analysis. Computational timing is instead aggregated from durations already recorded during robustness cases 1--10, without rerunning any optimization or exact replay. This timing-only revision retains the completed casewise artifacts byte-for-byte, excludes the nominal case from timing averages, and reports the number of available observations for every category.
...
The nominal influent is the midpoint of each declared influent interval. Both optimization routes use the same free controls, bounds, objective priorities, and engineering constraints. The surrogate route and the primary direct route each begin at the componentwise center of the normalized seven-dimensional box. For the surrogate route, the former primal--dual-gap IPOPT continuation is not executed; the exact-QP active-set problem in the seven normalized controls is the primary solver.

The original projection QP is cold-solved to its acceptance tolerances at every distinct surrogate trial point. On a stable active set, response derivatives follow from the equality-plus-active-inequality QP KKT sensitivity system after rank and conditioning checks; an active-set change triggers a fresh QP and derivative reconstruction. If exact active-set derivatives cannot be constructed, deterministic value-only COBYQA continues within the normalized control bounds. Every distinct fallback trial receives a new cold solve and complete audit of the same projection QP; the fallback introduces neither an approximate projection nor finite-difference derivatives. Its selected best feasible visited point is cold-projected again. A surrogate candidate receives the strong first-order tier only if its independently recomputed lower-QP and upper KKT audits pass. Otherwise, complete feasible no-descent polls at both normalized radii $10^{-3}$ and $10^{-4}$ can qualify it only at finite resolution under protocol v3. After a winning poll step, up to 16 search-only exact-QP trials expand geometrically along that ray; a new complete poll is mandatory after every accepted move, so an acceleration trial never supplies certification evidence. Each completed poll must contain a feasible nonzero displacement; direction rank is diagnostic only. Any accepted fine-scale move forces the coarse poll to be repeated before certification, and the shared certification safety budget is 10,000 exact-QP evaluations. A failed or incomplete poll leaves stationarity unresolved. First-order stationarity is not a certificate of local minimality, and the finite poll is neither KKT/Clarke stationarity nor proof of local or global optimality.

The primary direct mechanistic solve retains its three-stage smoothing continuation from the center and its full-space endpoint audit. If that primary attempt fails, at most one recovery may be run from the same case's certified surrogate control, using the unchanged three-stage sequence. No recovery is run after a successful primary attempt or when the surrogate candidate is uncertified. Thus recovery is a disclosed dependency on the surrogate route, not an additional independent basin search. A validated feasible but budget-limited or stationarity-unresolved point remains a reportable incumbent. If a route produces no validated candidate, that failure remains in the case denominator and execution proceeds.

For each available selected candidate $k\in\{S,M\}$, an exact nonsmooth mechanistic replay produces the common-reference response $\chi_{{\rm ref},k}$. The optimizer-native response is $\chi_{{\rm nat},S}=\widehat\chi_S$ for the surrogate route and $\chi_{{\rm nat},M}=\chi_{{\rm sm},M}$ for the direct route. Raw and projected surrogate responses are also evaluated at both controls so prediction error is not confounded with decision difference:
\begin{align}
...
\end{align}
Each valid, physically audited exact replay retains its exact objective and objective components even when the exact response is engineering-infeasible. Such a candidate is reported but cannot enter a paired route comparison. Accordingly, $\Delta J_{S-M}$ is reported only when both candidates have valid, engineering-feasible exact replays. It compares selected feasible incumbents and is not called regret because neither route supplies a global certificate. A symmetric relative objective difference and normalized control RMS and maximum differences accompany it.

The two exact replays use the declared primary and alternate reference initializations for every selected decision. Their generation-scale and frozen-state-scale root discrepancies must each be no greater than $10^{-6}$. Categorical branch assignments must agree except that a mismatch within either start's declared boundary-ambiguity band is retained as a qualifier rather than used alone to reject an otherwise agreeing root. Exact mass-conservation, non-negativity, residual, domain, and reduced-stability audits must also pass. Proximity to a branch boundary prevents an unqualified differentiable-stationarity or smooth-equivalence interpretation. Whole-test smooth/reference validation is retired. The 1,000 untouched test inputs remain reserved for raw and projected surrogate prediction assessment, and any partial legacy equivalence rows are excluded from analysis.
...
Optimizer-native & Every available selected candidate & -- & -- & -- & -- & --\\
...
Optimizer-native objective $J(\vartheta_M,\chi_{{\rm nat},M})$ & n.a. & --\\
...
The nominal discussion will trace raw, projected, optimizer-native, and exact profiles from the mixer through each reactor and down the Clarifier. It will identify which inventories required correction, which controls or engineering constraints were active, whether either replay carries a branch-boundary qualifier, and whether the two exact-reference objectives differ materially.

\subsection{Casewise exact common-reference validation}

\begin{table}[htbp]
\centering
\caption{Exact nonsmooth common-reference adjudication of every available selected candidate (at most 22). Counts use the actual candidate denominator; the untouched test block is not part of this validation.}
\label{tab:common_reference_results}
\begin{tabular}{lrrr}
\toprule
Adjudication count & Surrogate & Direct & Total\\
\midrule
...
Common-reference comparison eligible & -- & -- & --\\
\bottomrule
\end{tabular}
\end{table}

This table does not claim whole-domain smooth/nonsmooth equivalence. Branch-boundary-qualified rows remain eligible when the required two-start exact replay, physical and stability audits, and engineering checks pass. A valid exact objective remains reported if the engineering check fails, but its paired-comparison eligibility is false. Candidate absence, direct failure, invalid replay, and comparison ineligibility are retained separately rather than collapsed into a favorable denominator.

\subsection{Influent scenarios and optimization comparison}

\begin{table}[htbp]
\centering
\caption{Required casewise influent-scenario comparison. The full table contains ten rows. Here $\mathrm{RMS}_{\rm proj,S}=\|e_{\rm proj,S}\|_2/\sqrt{n_\chi}$, $\mathrm{RMS}_{\rm nat,M}=\|e_{\rm nat,M}\|_2/\sqrt{n_\chi}$, and the time ratio is complete mechanistic-route time divided by complete surrogate-route time. Reference objective comparisons are shown only for common-reference-eligible pairs.}
\label{tab:sensitivity_results}
\small
\begin{tabular}{lrrrrrr}
\toprule
Case & $J_{S,\rm ref}$ & $J_{M,\rm ref}$ & $\Delta J_{S-M}$ & $\mathrm{RMS}_{\rm proj,S}$ & $\mathrm{RMS}_{\rm nat,M}$ & $t_M/t_S$\\
\midrule
1--10 & -- & -- & -- & -- & -- & --\\
\bottomrule
\end{tabular}
...
\begin{tabular}{lllll}
\toprule
Case & Surrogate outcome/tier & Direct outcome/recovery & Selected $n$ & Common-reference outcome\\
\midrule
Nominal & -- & -- & -- & --\\
...
Median fallback exact-QP evaluations & -- & n.a.\\
Strong-KKT certificates / selected candidates & -- & n.a.\\
...
The projection has a deliberately limited role: it enforces the declared linear network physics and non-negativity while making the smallest standardized change to the raw prediction. The production surrogate optimizer resolves that exact lower QP within a seven-control search and audits its primal--dual KKT conditions independently; it does not embed the former gap-continuation NLP. Analytical active-set sensitivities remain primary, while a deterministic value-only fallback preserves a reportable feasible incumbent when those sensitivities are unavailable. Strong KKT evidence and finite-direction, finite-resolution two-scale feasible-poll evidence are reported as distinct convergence tiers. Direction rank in the latter is diagnostic rather than a false full-space requirement at an active constrained point, and neither an incomplete poll nor a budget-limited result is overstated as KKT/Clarke stationary or locally/globally optimal. Nonlinear kinetics, settling, and objective comparability are adjudicated by replaying each available selected candidate on the same exact nonsmooth mechanistic model. Branch-boundary ambiguity qualifies that comparison rather than rejecting it by itself. Numerical conclusions will be finalized only after all required tables are populated under the dated, explicitly disclosed protocol revisions above; any further change must likewise be recorded with its timing and evidential context.
...
The study record will contain the 5,000 accepted mechanistic input--response states; the complete accepted and rejected candidate-attempt ledger; immutable candidate-to-slot, development/test, and fold membership; fitted coefficients and scales; raw and projected test predictions; every nominal and influent-scenario optimization record and convergence tier; the archived, analysis-excluded v1 poll outputs and v2 migration record; conditional direct-recovery provenance; casewise exact common-reference states; raw, projected, optimizer-native, and exact physical-audit ledgers; and timing summaries.
